...
function collidefront(creature) {
    ...
    for (fstblocX; fstblocX < Maps.lengths; fstblocX++) {
...
function collideback(creature) {
    let fstblocX = Math.trunc((creature.Left - divka.Left - 1) / 36)

    let fstblocY = Math.trunc((creature.Top + 63) / 36)
    let sndblocY = Math.trunc((creature.Top) / 36)
    let thrdblocY = Math.trunc((creature.Top + 35) / 36)
...
function collidebottom(creature) {
    let fstblocX = Math.trunc((creature.Left - divka.Left + 1) / 36)
    let sndblocX = Math.trunc((creature.Left - divka.Left + 30) / 36)

    let fstblocY = Math.trunc((creature.Top + 65) / 36)
    for (fstblocY; fstblocY < 15; fstblocY++)
        if ((Maps[fstblocX][fstblocY] != undefined) || (Maps[sndblocX][fstblocY] != undefined)) {
            return fstblocY * 36 - 64
        }
    return 1000
}
